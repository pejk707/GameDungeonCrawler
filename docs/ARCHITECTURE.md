...
| Консоль | `IConsole` + `WinConsole`/`PosixConsole`/`ScriptedConsole` | `ScriptConsole` — обёртка поверх любой консоли: команды из файла (режим `--script`); `MemoryConsole` — для тестов | Один механизм для тестов и сценариев |
