...
12. [Отличия реализации от проекта](#12-отличия-реализации-от-проекта)
...
## 12. Отличия реализации от проекта

Реализация (Milestone 3) следует этому проекту. По ходу работы несколько решений уточнились:

| Где | В проекте | В реализации | Почему |
|---|---|---|---|
| `InteractionSystem` | Берётся первое взаимодействие с подходящим триггером | Берётся первое, чьи условия выполнены. Если не подошло ни одно — первое подходящее, и срабатывает его `onFail` | Объект может по-разному отвечать на повторное действие («чаша уже горит»), и это описывается данными, без кода |
| Требования для триггера `Light` | Фонарь должен гореть всегда | Фонарь нужен, только если действие тратит масло | Чашу Сердца нужно разжечь углями даже с пустым фонарём: иначе в конце возможен тупик |
| Переходы между режимами | Запрос `ctx.pendingState` обрабатывает режим в `handleInput` | Так и есть, а `Game::settle` дополнительно обрабатывает запросы, сделанные в `onEnter` | После боя в комнате может остаться ещё один враг, и `ExplorationState::onEnter` сразу начинает с ним бой |
| Описание комнаты при переходе | `MovementSystem` вызывает `RoomDescriber` | `GoHandler` вызывает `move()` → `RoomDescriber::describe()` → `arrive()` | Системы не зависят от модуля `ui` (правило из раздела 2.1) |
| ASCII-карта | Карта посещённых комнат | Карта текущей зоны и её известных соседей | Весь мир не помещается в 80 символов ширины |
| Формат `save.json` | Инвентарь — объект `{id: count}` | Массив пар `[id, count]` | Сохраняется порядок, в котором найдены предметы |
| Новые поля состояния | — | `EnemyInstance::engaged` (бой уже начинался — паттерн не перезапускается), `GameContext::lastCombatRoom` (после бегства новая комната описывается целиком) | Нужны для корректного повторного боя и описания после бегства |
| Консоль | `IConsole` + `WinConsole`/`PosixConsole`/`ScriptedConsole` | `ScriptConsole` — обёртка поверх любой консоли с задержками ввода (режимы `--script` и `--demo`); `MemoryConsole` — для тестов | Один механизм для тестов, сценариев и записи видео |
